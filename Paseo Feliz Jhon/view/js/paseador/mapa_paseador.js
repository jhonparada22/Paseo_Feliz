...
// Igual pero para el aviso local de 300m ("Llegando a Punto X")
const avisosLocalesCerca = new Set();
...
function cargarRutaDeHoy() {
    return fetch(API + 'obtener_ruta.php?modo=hoy')
        ...
        .then(data => {
            ...
            // Si la ruta ya estaba EN CURSO en el servidor (ej. el paseador
            // recargó la página a mitad del paseo), reactivar el seguimiento:
            // sin esto el GPS quedaba apagado en silencio y el cliente dejaba
            // de ver la posición sin que el paseador lo notara.
            if (r.estado === 'en_curso' && !paseoActivo) {
                reanudarTrasRecarga();
            } else if (r.estado === 'pausada' && !paseoActivo) {
                document.getElementById('btnPaseoLabel').textContent = 'Reanudar paseo';
            }
...
function renderRouteOnMap() {
    ...
    RUTA.paradas.forEach((p, i) => {
        ...
        const isOmitida = p.estado === 'omitida';
        const isCurrent = i === paradaActual && paseoActivo && !isOmitida;
        const sz = isCurrent ? 40 : 34;
        const icon = L.divIcon({
            html: `<div style="
                width:${sz}px;height:${sz}px;border-radius:50%;
                background:${(isDone || isOmitida) ? '#94a3b8' : p.color};
                ...
                ${(isDone || isOmitida) ? 'opacity:.55;' : ''}
            ">${isDone ? '✓' : (isOmitida ? '✖' : p.label)}</div>`,
...
function renderParadas() {
    ...
    RUTA.paradas.forEach((p, i) => {
        ...
        if (isOmitida) { estadoCls = 't-hecho';  estadoLbl = '✖ Cancelado'; }
        ...
        div.innerHTML = `
            <div class="parada-left">
                <div class="parada-dot" style="background:${(isDone || isOmitida) ? '#94a3b8' : p.color};${isCurrent ? 'box-shadow:0 0 0 3px ' + p.color + '40' : ''}">
                    ${isDone ? '✓' : (isOmitida ? '✖' : p.label)}
                </div>
                ${i < RUTA.paradas.length - 1 ? '<div class="parada-line"></div>' : ''}
            </div>
            <div class="parada-body" style="opacity:${(isDone || isOmitida) ? '.5' : '1'}">
...
// Retoma un paseo que el servidor tiene "en curso" sin volver a notificarlo:
// mismo efecto visual que iniciarPaseo() pero avisando que es una reanudación.
function reanudarTrasRecarga() {
    paseoActivo = true;
    document.getElementById('btnPaseoIcon').className  = 'fas fa-pause';
    document.getElementById('btnPaseoLabel').textContent = 'Pausar paseo';
    document.getElementById('btnPaseo').className        = 'btn-iniciar pause';
    ...
    startGPS();
    startTimer();
    showNotif('🔄 Paseo en curso re-sincronizado — GPS reactivado', 'info');
}
    ...
    .then(data => {
        ...
        if (data.eventos && data.eventos.length) {
            data.eventos.forEach(ev => {
                ...
                // También como notificación del sistema (el paseador suele
                // llevar el teléfono en el bolsillo con la app minimizada)
                if (typeof pfNotificar === 'function') {
                    pfNotificar('📍 Paseo Feliz', `Estás en el punto ${p.label} — confirma ${accion} en la app`, 'parada-' + ev.id_parada);
                }
...
function updatePosition(lat, lng) {
    ...
    // Aviso de proximidad en el front (300m → notificación local, UNA vez
    // por parada: antes se repetía con cada ping del GPS mientras estuviera
    // cerca, tapando la pantalla con la misma notificación)
    const target = RUTA.paradas[paradaActual];
    if (target && target.estado === 'pendiente' && !avisosLocalesCerca.has(target.id)) {
        const dist = haversine(lat, lng, target.lat, target.lng);
        if (dist < 0.3) {
            avisosLocalesCerca.add(target.id);
...
function updateProgress() {
    ...
    // Las paradas canceladas (omitidas) cuentan como resueltas: sin esto el
    // progreso nunca llegaba al 100% y el paseo no se finalizaba jamás
    const resueltas = RUTA.paradas.filter(p => p.estado === 'completado' || p.estado === 'omitida').length;
    const pct  = Math.round(resueltas / total * 100);
    ...
    // Si todas las paradas están resueltas, finalizar paseo
    if (resueltas === total && total > 0 && paseoActivo) {
        finalizarPaseo();
    }
}
...
function botonesPaseo(p) {
    const chat = `<button class="pi-btn chat" title="Chat con ${p.cliente}" onclick="abrirChatCliente(${p.id_cliente})"><i class="fas fa-comment-alt"></i></button>`
        + (p.id_cliente ? `<button class="pi-btn chat" title="Ver perfil de ${p.cliente}" onclick="window.location.href='perfil_paseador.php?id=${p.id_cliente}'"><i class="fas fa-user"></i></button>` : '');
    ...
    if (p.estado === 'recogido') {
        return `
            ...
            <button class="pi-btn cancelar" onclick="abrirModalCancelar(${p.id_pedido})">Cancelar</button>
            ${foto}${chat}`;
    }
    return `
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (localStorage.getItem('paseoIniciado') === 'true') {
        ...
        // Esperar a que la ruta cargue antes de iniciar (si la reanudación
        // automática ya activó el paseo, no volver a iniciar el GPS/timer)
        setTimeout(() => { if (RUTA.id && !paseoActivo) { iniciarPaseo(); fitRoute(); } }, 1500);
    }
});
