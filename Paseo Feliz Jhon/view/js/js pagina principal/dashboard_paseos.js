...
(function () {
    ...
    // Estilos de "añadir otra mascota al servicio" (no hay CSS externo
    // para esto, se inyecta una sola vez).
    (function inyectarEstiloOtraMascota() {
        if (document.getElementById('dz-om-estilos')) return;
        const style = document.createElement('style');
        style.id = 'dz-om-estilos';
        style.textContent =
    ...
    // Si el usuario elige otra mascota desde la tarjeta de "Acciones", todo
    // el dashboard (mapa, resumen, timeline, etc.) sigue mostrando ESA
    // mascota hasta que vuelva a cambiarla — no solo el modal de instrucciones.
    ...
    async function cargarEstado() {
        const url = API + 'estado_servicio_paseos.php' + (idMascotaElegida ? '?id_mascota=' + idMascotaElegida : '');
        ...
        S = data.servicio;
        // Recordamos la mascota que se está mostrando, para que el polling
        // (cada 15s) siga mostrando la MISMA mascota en vez de saltar a la
        // "más reciente" cada vez que se refresca.
    ...
    function render() {
        ...
        if (estado === 'paseo_en_curso') {
            ...
            col1 = cardMapa(true) + cardInstrucciones() + cardOtraMascota();
            ...
            col1 = cardMapa(false) + cardProximoPaseo() + cardOtraMascota();
            ...
            col1 = cardMapa(false) + cardDireccion() + cardOtraMascota();
            col2 = cardEstadoServicio() + cardResumen() + cardHistorial('Historial reciente');
    ...
    // "Añadir a otra mascota al servicio" — vive aquí (col1, debajo de la
    // dirección) en vez del banner fijo de inicio_cliente.js, para que no
    // se pierda cuando el dashboard reemplaza la vitrina normal. Usa la
    // misma variable global "membresias" que ya carga inicio_cliente.js
    // (ambos scripts comparten el mismo scope de página). Al hacer clic se
    // abre un mini-modal con 2 opciones: unirse al servicio actual (modo
    // exprés, hereda días/horario/dirección/paseador) o configurar uno
    // nuevo desde cero.
    function cardOtraMascota() {
        if (typeof membresias === 'undefined' || !membresias.mascotas) return '';

        const disponibles = membresias.mascotas.filter(function (m) { return !m.paseos; });
    ...
    function abrirModalAgregarMascota() {
        ...
        const p = S.pedido;
        const ocupadas = membresias.mascotas.filter(function (m) { return m.paseos; })
        ...
        document.getElementById('dz-add-normal').addEventListener('click', function () {
            ...
            abrirWizardPaseos({ ocupadas: ocupadas });
        });
    }
    ...
    function cardAcciones() {
        ...
        // Selector de mascota: solo si hay más de 1 con Paseos activo. Al
        // cambiarlo, TODO el dashboard (mapa, resumen, timeline, etc.)
        // cambia a mostrar esa mascota — no solo las instrucciones.
        const mascotasConPaseos = (typeof membresias !== 'undefined' && membresias.mascotas)
            ? membresias.mascotas.filter(function (m) { return m.paseos; })
            : [];
        const selectorMascota = mascotasConPaseos.length > 1
            ? '<label class="dz-label" style="margin-bottom:4px;display:block;">Mascota</label>' +
              '<select id="dz-sel-mascota-acciones" class="dz-textarea" style="cursor:pointer;margin-bottom:14px;">' +
                  mascotasConPaseos.map(function (mm) {
    ...
    function conectarAcciones() {
        ...
        const selMascotaAcciones = document.getElementById('dz-sel-mascota-acciones');
        if (selMascotaAcciones) {
            selMascotaAcciones.addEventListener('change', function () {
                idMascotaElegida = parseInt(this.value, 10);
                firmaRender = ''; // forzar repintado aunque los datos "parezcan" iguales
    ...
    function abrirModalInstrucciones() {
        ...
        const m = abrirModalDz(
            '<button class="dz-modal-x" data-cerrar><i class="ph ph-x"></i></button>' +
            '<h3 class="dz-h3"><i class="ph ph-clipboard-text"></i> Instrucciones del paseo de ' + esc(p.mascota) + '</h3>' +
    ...
    function abrirModalPerfil() {
        ...
        abrirModalDz(
            ...
            (a.id_usuario
                ? '<a class="dz-btn dz-btn-borde dz-btn-full" style="margin-top:8px" href="perfil.php?id=' + a.id_usuario + '"><i class="ph ph-user"></i> Ver perfil completo</a>'
                : '')
        );
    }
})();