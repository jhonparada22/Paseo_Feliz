...
(function () {
    ...
    window.abrirWizardAdiestramiento = async function (opts) {
        opts = opts || {};
        ...
        try {
            ...
            if (EXPRESS) {
                ...
                W.cantidad_sesiones  = b.cantidad_sesiones || W.cantidad_sesiones;
    ...
    function renderPaso(n) {
        pasoActual = n;
        $('#wza-subtitulo').innerHTML = EXPRESS && n === 1
    ...
    function renderPaso1() {
        ...
        {
            const btnReg = $('#wza-cuerpo [data-registrar-mascota]');
        ...
        botonesFooter(false, 'Continuar con ubicación <i class="ph ph-arrow-right"></i>', () => {
            ...
            if (!W.cantidad_sesiones || W.cantidad_sesiones < MIN_SESIONES_MES) return alertaPaso(`Ingresa cuántas sesiones al mes quieres (mínimo ${MIN_SESIONES_MES}).`);
    ...
    // Cantidad de sesiones, días, horario, dirección y entrenador se
          ...
          <div>
            <div class="wz-banner-info" style="margin-bottom:14px">
              ...
              <div>
                ...
                ${W.cantidad_sesiones} sesiones al mes · ${diasTxt} · ${W.franja} ·
                misma dirección y mismo entrenador.
              </div>
            </div>
    ...
    function abrirFormRegistrarMascota() {
        let ov = document.getElementById('wza-reg-overlay');
        if (ov) ov.remove();
        ov = document.createElement('div');
        ov.id = 'wza-reg-overlay';
        ov.innerHTML = `
        <div class="wz-reg-box">
          <div class="wz-reg-head">
            <h3><i class="ph ph-paw-print"></i> Registrar nueva mascota</h3>
            <button class="wz-close" id="wza-reg-cerrar"><i class="ph ph-x"></i></button>
          </div>
          <div class="wz-alerta-error" id="wza-reg-error"></div>
          <div class="wz-campo">
            <label>Nombre de la mascota *</label>
            <input type="text" id="wza-reg-nombre" maxlength="100" placeholder="Nombre del peludito">
          </div>
          <div class="wz-form-fila">
            <div class="wz-campo">
              <label>Raza</label>
              <input type="text" id="wza-reg-raza" maxlength="80" placeholder="Ej: Golden Retriever">
            </div>
            <div class="wz-campo">
              <label>Edad (años)</label>
              <input type="number" id="wza-reg-edad" min="0" max="30" placeholder="Ej: 3">
            </div>
          </div>
          <div class="wz-campo">
            <label>Foto de la mascota <span style="font-weight:400;color:#94a3b8">(opcional)</span></label>
            <input type="file" id="wza-reg-avatar" accept="image/*">
          </div>
          <div class="wz-campo">
            <label>Biografía canina</label>
            <textarea id="wza-reg-bio" rows="3" maxlength="1000" placeholder="Cuéntanos su historia..."></textarea>
          </div>
          <div class="wz-campo">
            <label>Enfermedades y/o discapacidades</label>
            <textarea id="wza-reg-enf" rows="2" maxlength="1000" placeholder="Ninguna / Alergias..."></textarea>
          </div>
          <button class="wz-btn wz-btn-prim" id="wza-reg-guardar" style="width:100%"><i class="ph ph-check"></i> Registrar mascota</button>
        </div>`;
        document.querySelector('#wizard-adiestramiento .wz-modal').appendChild(ov);

        document.getElementById('wza-reg-cerrar').addEventListener('click', () => ov.remove());
        ov.addEventListener('click', e => { if (e.target === ov) ov.remove(); });

        document.getElementById('wza-reg-guardar').addEventListener('click', async () => {
            const nombre = document.getElementById('wza-reg-nombre').value.trim();
            const err = document.getElementById('wza-reg-error');
            if (!nombre) {
                err.textContent = '⚠ El nombre de la mascota es obligatorio.';
                err.classList.add('visible');
                return;
            }
            const btn = document.getElementById('wza-reg-guardar');
            btn.disabled = true;
            btn.innerHTML = '<span class="wz-spinner"></span> Registrando...';

            const fd = new FormData();
            fd.append('nombre_mascota', nombre);
            fd.append('raza', document.getElementById('wza-reg-raza').value.trim());
            fd.append('edad', document.getElementById('wza-reg-edad').value);
            fd.append('biografia_canina', document.getElementById('wza-reg-bio').value.trim());
            fd.append('enfermedades_discapacidades', document.getElementById('wza-reg-enf').value.trim());
            const archivo = document.getElementById('wza-reg-avatar').files[0];
    ...
    function alertaPaso(msg) {
        ...
        if (!al) {
            ...
            $('#wizard-adiestramiento .wz-modal').appendChild(al);
        }
        al.textContent = '⚠ ' + msg;
