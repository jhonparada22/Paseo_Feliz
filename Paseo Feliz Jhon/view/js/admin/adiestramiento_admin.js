// ══════════════════════════════════════════════════════════════
// ADIESTRAMIENTO_ADMIN.JS — Gestión de pedidos de adiestramiento
// Muestra los pedidos reales (pedidos_adiestramiento) y permite
// asignarlos a un entrenador (paseador, mismo rol) y sus días de
// sesión (cronograma_adiestramiento). Mismo patrón que paseos_admin.js.
// ══════════════════════════════════════════════════════════════
const API = '../../../model/';
...
function estadoPedido(p) {
    ...
    if (p.asignacion)              return { lbl: 'Con entrenador',     cls: 'b-completado', dot: '#25D366' };
...
async function cargarPedidos() {
    try {
        const r = await fetch(API + 'obtener_pedidos_adiestramiento.php?estado=todos');
...
// ── Alertas (pedidos pagados sin entrenador) ───────────────────
function renderAlertas() {
    ...
    strip.innerHTML = `
        ...
        <div style="flex:1">
            <div class="al-title">${sinAsignar.length} pedido(s) pagado(s) sin entrenador asignado</div>
    ...
    lista.forEach(p => {
        ...
        div.innerHTML = `
            ...
            <div class="pc-body">
                ...
                <div class="pc-sub">Pedido #${p.id_pedido} · ${p.cantidad_sesiones} sesiones/mes</div>
                <div class="pc-tags">
                ...
                ${p.asignacion ? `<div class="pc-meta" style="color:#15803d"><i class="fas fa-chalkboard-user"></i> ${p.asignacion.paseador} (${diasTexto(p.asignacion.dias)})</div>` : ''}
...
function renderDetalle(id) {
    ...
    dp.innerHTML = `
        ...
        <div class="dp-section">
            ...
            <div class="dp-row"><i class="fas fa-receipt"></i> ${p.cantidad_sesiones} sesiones al mes · <strong>${cop(p.total)}</strong></div>
            <div class="dp-row"><i class="fas fa-clock"></i> ${p.duracion_min} min por sesión</div>
        ...
        <div class="dp-section">
            <div class="dp-label">Ubicación de las sesiones</div>
        ...
        <div class="dp-section">
            <div class="dp-label">Entrenador</div>
            ${p.asignacion
                ? `<div class="dp-row" style="color:#15803d"><i class="fas fa-chalkboard-user"></i> ${p.asignacion.paseador} — ${diasTexto(p.asignacion.dias)}</div>`
                : `<div class="dp-row" style="color:var(--muted)"><i class="fas fa-circle-info"></i> Aún sin asignar a un entrenador</div>`}
            ${p.estado === 'cancelado' ? `
            <button class="btn-primary" style="width:100%;margin-top:10px;background:#dcfce7;color:#15803d;border:1px solid #86efac" onclick="abrirModalAsignar(${p.id_pedido})">
                <i class="fas fa-rotate-left"></i> Reactivar y asignar entrenador
            </button>` : `
            <button class="btn-primary" style="width:100%;margin-top:10px" onclick="abrirModalAsignar(${p.id_pedido})">
                <i class="fas fa-calendar-plus"></i> ${p.asignacion ? 'Cambiar entrenador / días' : 'Asignar entrenador'}
            </button>
...
function abrirModalAsignar(idPedido) {
    ...
    sel.innerHTML = '<option value="">— Seleccionar entrenador —</option>' +
...
async function confirmarAsignacion() {
    ...
    if (!idPaseador) { showToast('Selecciona un entrenador', 'error'); return; }
    ...
    try {
        const r = await fetch(API + 'guardar_cronograma_adiestramiento.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id_pedido: idPedido, id_paseador: idPaseador, dias }),
        });
        const data = await r.json();
...
async function confirmarCancelarServicio() {
    ...
    btn.disabled = true;
    try {
        const r = await fetch(API + 'cancelar_pedido_adiestramiento.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
