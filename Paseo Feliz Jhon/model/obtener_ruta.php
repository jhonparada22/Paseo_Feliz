...
// Acceso directo por id_ruta (sin modo): la ruta trae teléfonos, direcciones
// y notas médicas de los clientes, así que solo pueden verla el admin, el
// paseador dueño de la ruta o un cliente que aparezca como parada en ella.
if ($modo !== 'hoy' && $modo !== 'cliente') {
    $esAdmin = isset($_SESSION['usuario_admin']) && $_SESSION['usuario_admin'] === true;
    if (!$esAdmin) {
        $idUsuario = (int)$_SESSION['usuario_id'];
        $chk = $conn->prepare(
            "SELECT 1 FROM rutas r
             ...
             WHERE r.id_ruta = ?
               AND (p.id_usuario = ? OR EXISTS (
                     ...
                     WHERE rp.id_ruta = r.id_ruta AND rp.id_usuario_cliente = ?
                   ))
             LIMIT 1"
        );
        $chk->bind_param("iii", $idRuta, $idUsuario, $idUsuario);
        $chk->execute();
        $autorizado = $chk->get_result()->num_rows > 0;
        $chk->close();
        if (!$autorizado) responder(false, [], 'No autorizado para ver esta ruta.');
    }
}
