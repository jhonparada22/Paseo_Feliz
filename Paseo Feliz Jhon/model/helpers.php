...
function estadoDerivadoPedido($horaRecogida, $horaEntrega, $horaCancelacion) {
    ...
    if ($horaRecogida)    return 'recogido';
    return 'pendiente';
}

/**
 * Borra las evidencias de paseo (foto + fila en BD) con más de $dias días.
 * Se llama de forma oportunista en cada subida y también puede colgarse de
 * un cron real del hosting (ver model/limpiar_evidencias_paseo.php) para
 * que no dependa de que alguien suba una foto para disparar la limpieza.
 * Devuelve cuántas se borraron.
 */
function limpiarEvidenciasVencidas($conn, $dias = 7) {
    $stmt = $conn->prepare(
        "SELECT id_evidencia, url FROM evidencias_paseo WHERE creado_en < DATE_SUB(NOW(), INTERVAL ? DAY)"
    );
    $stmt->bind_param("i", $dias);
    $stmt->execute();
    $res = $stmt->get_result();
    $vencidas = [];
    while ($row = $res->fetch_assoc()) $vencidas[] = $row;
    $stmt->close();

    if (!$vencidas) return 0;

    $dirEvidencias = __DIR__ . '/../view/assets/evidencia/';
    foreach ($vencidas as $v) {
        $archivo = $dirEvidencias . basename($v['url']);
        if (is_file($archivo)) @unlink($archivo);
    }

    $ids = array_column($vencidas, 'id_evidencia');
    $placeholders = implode(',', array_fill(0, count($ids), '?'));
    $stmt = $conn->prepare("DELETE FROM evidencias_paseo WHERE id_evidencia IN ($placeholders)");
...
/** Extrae la hora de inicio ("08:00") a partir de una etiqueta de franja ("8:00 a.m. – 11:00 a.m.") */
...
/**
 * true si pedidos_paseo ya tiene la columna hora_paseo (migración de hora
 * exacta). Permite desplegar el código antes de correr la migración sin
 * romper nada. Cachea el resultado por petición.
 */
...
/**
 * Finaliza la ruta si ya no le quedan paradas por resolver (todas están
 * completadas u omitidas/canceladas). Se llama tras cada entrega confirmada
 * y tras aprobar una solicitud de cancelación: sin esto, una ruta con
 * paradas canceladas nunca llegaba al 100% en el front y quedaba "en curso"
 * para siempre (el paseador aparecía "En ruta" todo el día en el panel admin).
 * Devuelve true si la ruta quedó finalizada en esta llamada.
 */
function finalizarRutaSiCompleta($conn, $idRuta) {
    if (!$idRuta) return false;
    $stmt = $conn->prepare(
        "SELECT COUNT(*) AS pendientes FROM ruta_paradas
         WHERE id_ruta = ? AND id_estado NOT IN (3,4) AND hora_cancelacion IS NULL"
    );
    ...
    // Solo cierra rutas vivas; las ya finalizadas/canceladas no se tocan
    $u = $conn->prepare(
        "UPDATE rutas SET id_estado = 4, fecha_fin_real = COALESCE(fecha_fin_real, NOW())
         WHERE id_ruta = ? AND id_estado IN (1,2,3)"
    );
    $u->bind_param("i", $idRuta);
    $u->execute();
    $cerrada = $u->affected_rows > 0;
    $u->close();
    return $cerrada;
}
 ...
 * Usa el UNIQUE KEY uq_ruta_activa para resolver la condición de carrera
 * entre el cronograma automático y la asignación manual del admin: si ambos
 * intentan crearla casi al mismo tiempo, uno gana el INSERT y el otro
 * reutiliza la que ganó. Devuelve ['id_ruta' => int, 'nueva' => bool].
 */
 ...
 * (recogida 7:00 → entrega 8:00 → recogida 8:30 → ...). Los pedidos sin
 * hora exacta van al final con el estimado de caminata de siempre (~80
 * m/min). Las paradas ya cerradas NO se tocan, conservan su 'orden' original.
 */
