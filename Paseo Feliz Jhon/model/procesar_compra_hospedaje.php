 ...
 * Hospedaje es una ESTADÍA con fecha Y HORA de entrada y salida — la
 * cantidad de noches se calcula sola por día de calendario (no la
 * manda el cliente).
 *
 * POST JSON esperado:
 * {
 *   "id_mascota": 6, "fecha_entrada": "2026-07-15T14:00", "fecha_salida": "2026-07-18T11:00",
...
// El cliente elige fecha Y HORA de entrada/salida (datetime-local del
// navegador: "2026-07-15T14:00"). Se normaliza a "Y-m-d H:i:s" para
// guardarla tal cual en la columna DATETIME.
function normalizarFechaHora($s) {
...
$fechaEntrada = normalizarFechaHora($data['fecha_entrada'] ?? '');
$fechaSalida  = normalizarFechaHora($data['fecha_salida'] ?? '');
...
// Si viene pedido_base, la nueva mascota HEREDA las fechas y la
// dirección del pedido activo (fuente de verdad: la BD) — lo que
// mande el cliente para esos campos se ignora. La nueva mascota paga
// y activa SU PROPIA membresía (son por mascota).
// ═══════════════════════════════════════════════════════════════════
...
if ($idPedidoBase > 0) {
    ...
    $stmt = $conn->prepare(
        "SELECT p.id_pedido, p.fecha_entrada, p.fecha_salida, p.comportamiento,
         ...
         WHERE p.id_pedido = ? AND p.id_usuario = ?
           ...
           AND DATE_ADD(m.fecha_inicio_hospedaje, INTERVAL 30 DAY) > $ahoraColombiaBase"
    );
    ...
    // La mascota nueva no puede tener ya un servicio de hospedaje activo
    $stmt = $conn->prepare(
        "SELECT id_pedido FROM pedidos_hospedaje
    ...
    $stmt->close();
    if ($yaActivaExp) {
        responder(false, [], 'Esta mascota ya tiene un servicio de hospedaje activo.');
    }

    // Heredar fechas y dirección del pedido base (fuente de verdad: la BD)
    $fechaEntrada = $pedidoBase['fecha_entrada'];
    $fechaSalida  = $pedidoBase['fecha_salida'];
...
// Validar fecha y hora: entrada en el futuro, salida después de la entrada
$dEntrada = DateTime::createFromFormat('Y-m-d H:i:s', $fechaEntrada);
$dSalida  = DateTime::createFromFormat('Y-m-d H:i:s', $fechaSalida);
if (!$dEntrada || $dEntrada < new DateTime()) {
    responder(false, [], 'La fecha y hora de entrada no son válidas (deben ser en el futuro).');
}
if (!$dSalida || $dSalida <= $dEntrada) {
    responder(false, [], 'La fecha y hora de salida deben ser posteriores a las de entrada.');
}
// La cantidad de noches se cuenta por DÍAS DE CALENDARIO, ignorando la
// hora exacta: entrar el lunes 2:00 p.m. y salir el miércoles 10:00 a.m.
// son 2 noches, no 1 día y pico.
$diaEntrada = new DateTime($dEntrada->format('Y-m-d'));
$diaSalida  = new DateTime($dSalida->format('Y-m-d'));
$cantidadNoches = (int)$diaEntrada->diff($diaSalida)->days;
...
try {
    ...
    $telegram->enviarMensajePagos(
        ...
        "📆 <b>Días solicitados:</b> " . $cantidadNoches . " noches (" . date('d/m/Y g:i a', strtotime($fechaEntrada)) . " → " . date('d/m/Y g:i a', strtotime($fechaSalida)) . ")\n" .
