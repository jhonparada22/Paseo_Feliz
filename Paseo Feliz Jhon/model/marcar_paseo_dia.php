 ...
 *     <- ya NO cancela directo: crea una solicitud en solicitudes_cancelacion
 *        pendiente de aprobación del admin (ver resolver_cancelacion.php).
 ...
 *   cancelar            requiere hora_entrega NULL y que no haya ya una
 *                       solicitud pendiente para ese pedido
 *   entregar_grupal     requiere que TODAS las recogidas del grupo estén confirmadas
 *
 * Efectos secundarios:
 *   - cancelar: NO cancela nada todavía — inserta en solicitudes_cancelacion
 *     y el paseo sigue activo hasta que el admin la resuelva (ver
 *     resolver_cancelacion.php, que sí notifica al cliente si se aprueba).
...
if ($accion === 'recogido') {
    ...
    // Verificar que el pedido siga activo (no entregado/cancelado ya)
    $stmt = $conn->prepare(
        "SELECT 1 FROM ruta_paradas
    ...
    $stmt->bind_param("ii", $idRuta, $idPedido);
    $stmt->execute();
    $yaEntregado = $stmt->get_result()->num_rows > 0;
    $stmt->close();
    if ($yaEntregado) responder(false, [], 'No se puede solicitar: el paseo ya fue entregado.');

    // Evitar solicitudes duplicadas para el mismo pedido
    $stmt = $conn->prepare(
        "SELECT 1 FROM solicitudes_cancelacion WHERE id_pedido = ? AND estado = 'pendiente' LIMIT 1"
    );
    $stmt->bind_param("i", $idPedido);
    $stmt->execute();
    $yaPendiente = $stmt->get_result()->num_rows > 0;
    $stmt->close();
    if ($yaPendiente) responder(false, [], 'Ya enviaste una solicitud de cancelación para este paseo; espera la respuesta del admin.');

    $idMascota = $pedido['id_mascota'] ? (int)$pedido['id_mascota'] : null;
    $stmt = $conn->prepare(
        "INSERT INTO solicitudes_cancelacion (id_ruta, id_pedido, id_paseador, id_cliente, id_mascota, motivo)
         VALUES (?, ?, ?, ?, ?, ?)"
    );
    $stmt->bind_param("iiiiis", $idRuta, $idPedido, $idPaseador, $idCliente, $idMascota, $motivo);
    $stmt->execute();
    $stmt->close();

    responder(true, ['estado' => 'solicitud_enviada', 'motivo' => $motivo],
