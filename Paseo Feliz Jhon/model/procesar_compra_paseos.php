 ...
 *   "dias_preferidos": "lun,mie,vie", "hora_paseo": "08:00",
 ...
 * }
 *
 * "pedido_base" es opcional: modo exprés "añadir otra mascota al servicio
 * activo" — si viene, la configuración se hereda del pedido indicado y lo
 * demás en el payload se ignora (ver bloque MODO EXPRÉS más abajo).
 *
...
// del pedido activo (cantidad de paseos, días, hora, duración,
...
if ($idPedidoBase > 0) {
    ...
    $stmt = $conn->prepare(
        "SELECT p.id_pedido, p.cantidad_paseos, p.modalidad, p.duracion_min, p.dias_preferidos,
         ...
         WHERE p.id_pedido = ? AND p.id_usuario = ?
           ...
           AND DATE_ADD(m.fecha_inicio_paseos, INTERVAL 30 DAY) > $ahoraColombiaBase"
    );
    ...
    // La mascota nueva no puede tener ya un servicio de paseos activo
    $stmt = $conn->prepare(
    ...
    $stmt = $conn->prepare(
        "SELECT p.id_pedido
         ...
         WHERE p.id_mascota = ? AND p.id_usuario = ?
           ...
           AND m.paseos = 1
           AND m.fecha_inicio_paseos IS NOT NULL
           AND DATE_ADD(m.fecha_inicio_paseos, INTERVAL 30 DAY) > $ahoraColombiaChk
         LIMIT 1"
    );
...
// Hora exacta del paseo: HH:MM entre 6:00 a.m. y 4:30 p.m., en pasos de 15
// min. La etiqueta franja_horaria se DERIVA de hora + duración (lo que
// mande el cliente en franja_horaria se ignora); si el navegador aún tiene
// el wizard viejo cacheado y solo manda la franja, se toma su hora de
// inicio para no rechazar la compra.
...
try {
    ...
    // La columna hora_paseo solo existe tras correr la migración de hora
    // exacta; si aún no corre, el pedido se guarda igual (solo con la
    // etiqueta de texto en franja_horaria).
    ...
    $stmt = $conn->prepare(
        "INSERT INTO pedidos_paseo
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, {$markHora}?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, 'pendiente_pago')"
    );
