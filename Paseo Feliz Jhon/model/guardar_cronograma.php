...
function validarPedido($conn, $idPedido) {
    ...
    // 'cancelado' se incluye a propósito: el admin debe poder reasignar al
    // cronograma un pedido que canceló por error o cuyas circunstancias
    // cambiaron, sin quedar bloqueado para siempre.
    if (!in_array($row['estado'], ['listo_para_asignar', 'en_validacion', 'pagado', 'cancelado'])) {
...
// quedaría en el cronograma pero seguiría mostrándose cancelado en el
// dashboard del cliente y con la membresía desactivada.
function reactivarSiCancelado($conn, $idPedido) {
    $s = $conn->prepare("SELECT estado, id_usuario, id_mascota FROM pedidos_paseo WHERE id_pedido = ?");
    $s->bind_param("i", $idPedido);
    $s->execute();
    $row = $s->get_result()->fetch_assoc();
    $s->close();
    ...
    $m = $conn->prepare("UPDATE membresias SET paseos = 1 WHERE id_usuario = ? AND id_mascota = ?");
