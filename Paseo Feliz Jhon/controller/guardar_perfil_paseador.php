...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$id_usuario) {
        header("Location: ../view/vistas/paseador/usuario_paseador.php");
        exit();
    }
    ...
    if ($mascota_accion === 'agregar') {
    ...
    } elseif ($mascota_accion === 'eliminar') {
        ...
        if ($id_mascota > 0) {
            ...
            if ($datos_mascota) {
                ...
                $stmt_del_pet = $conn->prepare("DELETE FROM mascota_usuario WHERE id_mascota = ? AND id_usuario = ?");
                $stmt_del_pet->bind_param("ii", $id_mascota, $id_usuario);
                $stmt_del_pet->execute();
                $stmt_del_pet->close();
            }
        }
